...
let type_to_string =
  (function
    | Content _ -> "cnt" | Directory -> "dir" | Release -> "rel" | Revision -> "rev" | Snapshot -> "snp")

let pp_id fmt ((scheme_version, o_type, id), _qualifiers) =
  Format.fprintf fmt "%d:%s:%s" scheme_version (type_to_string o_type) id

(* test content_identifier *)
let () =
  let test_cases =
    [| ("coucou\n", ((1, Content "sha1_git", "28d0af969b32e69a389087d7a267a2ecc05f1350"), []))
     ; ("coucoucou\n", ((1, Content "sha1_git", "ffacc412c4e5ff55cafccd6e58bc58072c17ff6b"), []))
    |]
  in
  Array.iter
    (fun (content, expected_identifier) ->
        let result = Swhid_compute.content_identifier content in
        let result =
        ...
        if not ok then
          Format.eprintf
            "error: expected identifier `%a` but got identifier `%a`@."
            pp_id expected_identifier pp_id result;
        assert ok )
    test_cases
...
let () =
    ...
    [| ( "741b2252a5e14d6c60a913c77a6099abe73a854a"
       ...
       , Some
           "Linux 2.6.14 release\n\
            ...
            =7VeT\n\
            -----END PGP SIGNATURE-----\n"
       , ((1, Release , "2b10839e32c4c476e9d94492756bb1a3e1ec4aa8" ), []))
     (* No author *)
     ; ( "9ee1c939d1cb936b1f98e8d81aeffab57bae46ab"
       ...
       , Some
           "This is the final 2.6.12 release\n\
            ...
            =sBHn\n\
            -----END PGP SIGNATURE-----\n"
       , ((1, Release, "26791a8bcf0e6d33f43aef7682bdb555236d56de"), [] ))
     (* No message *)
     ; ( "9ee1c939d1cb936b1f98e8d81aeffab57bae46ab"
       , Revision
       , "v2.6.12"
  ...
  Array.iter
    (fun (target, target_type, name, author, date, message, expected_identifier) ->
        let result =
          Swhid_compute.release_identifier target target_type name ~author date
            ~message
        in
        let result =
          match result with None -> assert false | Some result -> result
        ...
        if not ok then
          Format.eprintf
            "Test number: %d@.error: expected_identifier `%a` from release but \
             got identifier `%a`.@."
            count pp_id expected_identifier pp_id result;
        assert ok )
    test_cases
...
let () =
  ...
  in
  let test_cases =
    [| (( (1, Revision, "bc0195aad0daa2ad5b0d76cce22b167bc3435590"), [])
       ...
       , [||] )
     (* synthetic rev *)
     ; (( (1, Revision, "b2a7e1260492e344fab3cbf91bc13c91e05426fd"), [])
       ...
       , [||] )
     (* with extra headers *)
     ; (((1, Revision, "010d34f384fa99d047cdd5e2f41e56e5c2feee45"), [])
       , "85a74718d377195e1efd0843ba4f3260bad4fe07"
       , [ "01e2d0627a9a6edb24c37db45db5ecb31e9de808" ]
       , "Linus Torvalds <torvalds@linux-foundation.org>"
       ...
       |] )
     (* with_gpg_sign *)
     ; (((1, Revision,  "44cc742a8ca17b9c279be4cc195a93a6ef7a320e"), [])
       ...
       |] )
     (* No message *)
     ; (((1, Revision, "4cfc623c9238fa92c832beed000ce2d003fd8333"), [])
       , "b134f9b7dc434f593c0bab696345548b37de0558"
       , [ "689664ae944b4692724f13b709a4e4de28b54e57"
         ; "c888305e1efbaa252d01b4e5e6b778f865a97514"
       ]
       ...
       , [||] )
     (* Empty message *)
     ; ( ((1, Revision, "7442cd78bd3b4966921d6a7f7447417b7acb15eb"), [])
       , "b134f9b7dc434f593c0bab696345548b37de0558"
       , [ "689664ae944b4692724f13b709a4e4de28b54e57"
         ; "c888305e1efbaa252d01b4e5e6b778f865a97514"
       ]
  ...
  Array.iter
    (fun ( expected_identifier
         ...
         , extra_headers ) ->
        let result =
          Swhid_compute.revision_identifier directory parents ~author ~author_date
        ...
        if not ok then
          Format.eprintf
            "Test number: %d@.error: expected_identifier `%a` from revision but \
             got identifier `%a`@."
            count pp_id expected_identifier pp_id result;
        assert ok )
    test_cases
...
let () =
  let test_cases =
    [| (* empty directory *)
      (((1, Directory, "4b825dc642cb6eb9a060e54bf8d69288fbee4904"), []), [])
      ; (* swh example *)
      ( ((1, Directory, "d7ed3d2c31d608823be58b1cbe57605310615231"), [])
      , [ { Swhid_compute.typ = "file"
          ; permissions = 33188
          ; name = "README"
  ...
  Array.iter
    (fun (expected_identifier, entries) ->
        let result = Swhid_compute.directory_identifier entries in
        let result =
          match result with None -> assert false | Some result -> result
        in
        let ok = result = expected_identifier in
        if not ok then
          Format.eprintf
            "error: expected_identifier `%a` from directory but got identifier \
             `%a`@."
            pp_id expected_identifier pp_id result;
        assert ok )
    test_cases
...
let () =
  let test_cases =
    [| (* empty snapshot *)
      (((1, Snapshot, "1a8893e6a86f444e8be8e7bda6cb34fb1735a00e"), []), [])
      ; (* dangling branch *)
      (((1, Snapshot, "c84502e821eb21ed84e9fd3ec40973abc8b32353"), []), [ ("HEAD", None) ])
      ; (* unresolved *)
      ( ((1, Snapshot, "84b4548ea486e4b0a7933fa541ff1503a0afe1e0"), [])
      , [ ("foo", Some ("bar", "alias")) ] )
      ; (* all types*)
      ( ((1, Snapshot, "6e65b86363953b780d92b0a928f3e8fcdd10db36"), [])
      , [ ( "directory"
  ...
  Array.iter
    (fun (expected_identifier, branches) ->
        let result = Swhid_compute.snapshot_identifier branches in
        let result =
          match result with None -> assert false | Some result -> result
        in
        let ok = result = expected_identifier in
        if not ok then
          Format.eprintf
            "error: expected_identifier `%a` from snapshot but got identifier \
             `%a`@."
            pp_id expected_identifier pp_id result;
        assert ok )
    test_cases
...
let () =
  begin
    try
      let _id =
        Swhid_compute.directory_identifier
  ...
  begin
    try
      let _id =
        Swhid_compute.release_identifier "rust" Release "" ~author:None None
          ~message:None
      in
      assert false
    with Invalid_argument _ -> ()
  ...
  begin
    try
      let _id =
        Swhid_compute.revision_identifier "yo" [ "lo" ] ~author:"Bach"
  ...
  try
    let _id =
      Swhid_compute.snapshot_identifier [ ("do u know", Some ("bar", "àvin")) ]
    in
    assert false
  with Invalid_argument _ -> ()
